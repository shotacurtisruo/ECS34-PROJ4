...
// this a mock DSVReader that matches the actual interface

class tempDSVReader : public CDSVReader {
private:
    size_t CurrentRow;
...
public:
    ...
    tempDSVReader(const std::vector<std::vector<std::string>>& data) 
    ...
    bool ReadRow(std::vector<std::string> &row) {
        if (CurrentRow < tempData.size()) {
            row = tempData[CurrentRow];
            CurrentRow++;
            return true;
        }
        return false;
...
TEST_F(CSVBusSystemTest, StopCountAndAccess) {
    // Create mock readers
    auto stopReader = std::make_shared<tempDSVReader>(TestStopData);
    auto routeReader = std::make_shared<tempDSVReader>(TestRouteData);
...
TEST_F(CSVBusSystemTest, EmptyData) {
    ...
    auto emptyStopReader = std::make_shared<tempDSVReader>(std::vector<std::vector<std::string>>());
    auto emptyRouteReader = std::make_shared<tempDSVReader>(std::vector<std::vector<std::string>>());
